...
def train(args):
    ...
    log_file = '-{}-{}-reproduce-s{}'.format('drac_ucb', args.env_name, args.seed)
    ...
    for j in range(init_epoch, num_updates):
        ...
        # UCB decision
        _, update_epochs = ucb.select_ucb_hp()
        agent.ppo_epoch = update_epochs
        print(j, 'using K=', update_epochs)

        ucb.update_ucb_values(rollouts.returns)
