...
def train(args):
    ...
    log_file = '-{}-{}-reproduce-s{}'.format('drac_ts', args.env_name, args.seed)
    ...
    hp_list = [5, 3, 2, 1]
    ts = GaussianThompsonSampling(param_values=hp_list, window_size=args.window_length, lr=args.eta)
    ...
    for j in range(init_epoch, num_updates):
        ...
        # GTS decision
        _, update_epochs = ts.select_param()
